<template>
  <div v-if="isExperiencesPage" class="experience__page__menu">
    <div>
      ...
      <span> › </span>
      <router-link :to="{name: 'search', params:{name: experienceData.genre.category.search, case: 'category', id: experienceData.genre.category.id, typeWord: experienceData.genre.category.name }}" class="link__hover">{{ experienceData.genre.category.name }}</router-link>
